...
function App() {
  ...
  return <div className="min-h-screen overflow-hidden bg-[#081321] text-ink">
    ...
    {dashboardOpen && <Dashboard unlocked={dashboardUnlocked} ownerCode={ownerCode} onCodeChange={setOwnerCode} onUnlock={(code) => setDashboardUnlocked(code === OWNER_CODE)} onClose={() => setDashboardOpen(false)} />}
  </div>;
}
...
function Dashboard({ unlocked, ownerCode, onCodeChange, onUnlock, onClose }: { unlocked: boolean; ownerCode: string; onCodeChange: (value: string) => void; onUnlock: (code: string) => void; onClose: () => void }) {
  const [submissions, setSubmissions] = useState<Submission[]>([]);
  const [error, setError] = useState('');
  const unlock = (event: FormEvent) => { event.preventDefault(); if (ownerCode !== OWNER_CODE) { setError('Incorrect owner code.'); return; } setSubmissions(loadSubmissions().reverse()); setError(''); onUnlock(ownerCode); };
