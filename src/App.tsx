...
function App() {
  ...
  return <div className="min-h-screen overflow-hidden bg-[#081321] text-ink">
    ...
    {dashboardOpen && <Dashboard ownerCode={ownerCode} onCodeChange={setOwnerCode} onClose={() => setDashboardOpen(false)} />}
  </div>;
}
...
function Dashboard({ ownerCode, onCodeChange, onClose }: { ownerCode: string; onCodeChange: (value: string) => void; onClose: () => void }) {
  const [submissions, setSubmissions] = useState<Submission[]>([]);
  const [unlocked, setUnlocked] = useState(false);
  const [error, setError] = useState('');
  const unlock = (event: FormEvent) => { event.preventDefault(); if (ownerCode !== OWNER_CODE) { setError('Incorrect owner code.'); return; } setSubmissions(loadSubmissions().reverse()); setError(''); setUnlocked(true); };
